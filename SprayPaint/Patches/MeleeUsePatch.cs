    ...
    public static class MeleeUsePatch
    {
        ...
        internal static void OnPlayerAttackInvoker(UseableMelee __instance)
        {
            ...
            if (player.Player.equipment.itemID == 0 || !Main.Instance.Configuration.Instance.Vehicles
                                                 .Any(vehicle => vehicle.PaintedVehicles.Any(paint => paint.PaintID == player.Player.equipment.itemID) && vehicle.BaseVehicleID == hit.transform.GetComponent<InteractableVehicle>().id)) return;

            var new_vehicles =
                Main.Instance.Configuration.Instance.Vehicles.FirstOrDefault(vehicle =>
                    vehicle.BaseVehicleID == hit.transform.GetComponent<InteractableVehicle>().id);

            if (new_vehicles == null) return;
            {
                var new_vehicle =
                    new_vehicles.PaintedVehicles.FirstOrDefault(
                        vehicle => vehicle.PaintID == player.Player.equipment.itemID);
